...
public class GradeController {
				...
				if (studentRepository.existsById(studentId)) {
					if (subjectRepository.existsById(subjectId)) {
						if (teacherSubjectRepository.existsSubjectIdAndTeacherId(subjectId, teacherId) >= 1) {
							if (gradeService.existsTeacherSubjectStudent(teacherId, subjectId, studentId) >= 1) {
								...
								if (principal.getName().equals(user.getUsername()) || principal.getName().equals(student.getUsername())
									...
									if (subject.getSubjectForYear().equals(student.getClasss().getYear())) {
										...
										newGrade.setTeacherSubject(teacherSubject);
	...
	public ResponseEntity<?> getAllGradesForStudent(@PathVariable Integer studentId) {
		...
		if (studentRepository.existsById(studentId)) {
			...
			if (username.equals(student.getUsername()) || username.equals(student.getParent().getUsername())
				...
				List<GradeEntity> grades = new ArrayList<>();
				grades = gradeRepository.findByStudent(studentId);
				if (!grades.isEmpty()) {
	 ...
	 * 
	 * @RequestMapping(method = RequestMethod.GET, value =
	 * "/{teacherId}/getAllGradesForStudent/{studentId}/forSubject/{subjectId}")
	 * 
	 * @Secured({"ROLE_TEACHER", "ROLE_ADMIN"}) public ResponseEntity<?>
	 * getAllGradesForSubject(@PathVariable Integer teacherId, @PathVariable Integer
	 * subjectId, @PathVariable Integer studentId) {
	 * 
	 * String username = userService.getLoggedUser();
	 * 
	 * UserEntity user = userRepository.findById(1).get(); TeacherEntity teacher =
	 * teacherRepository.findById(teacherId).get();
	 * 
	 * if (studentRepository.existsById(studentId)) { if
	 * (subjectRepository.existsById(subjectId)) { if
	 * (username.equals(teacher.getUsername()) ||
	 * username.equals(user.getUsername())) { if
	 * (subjectRepository.existsById(subjectId)) { if
	 * (teacherRepository.existsById(teacherId)) { if
	 * (teacherSubjectRepository.existsSubjectIdAndTeacherId(subjectId, teacherId)
	 * >= 1) { if (tesubSubjectStudentService.existsTeacherSubjectStudent(teacherId,
	 * subjectId, studentId) >= 1) { } else { return new ResponseEntity<RESTError>(
	 * new RESTError(HttpStatus.NOT_FOUND.value(), "Student with id number " +
	 * studentId + " is not listening to subject with id number " + subjectId +
	 * " and teacher with id number " + teacherId), HttpStatus.NOT_FOUND); }
	 * List<GradeEntity> grades = new ArrayList<>(); grades = (List<GradeEntity>)
	 * gradeService.findAllGradesByTeacherSubjectStudent(subjectId, teacherId,
	 * studentId); if (!grades.isEmpty()) {
	 * logger.info("Viewed all grades for subject with id " + subjectId); return new
	 * ResponseEntity<List<GradeEntity>>(grades, HttpStatus.OK); } else { return new
	 * ResponseEntity<RESTError>(new RESTError(HttpStatus.NOT_FOUND.value(),
	 * "Grades not found") , HttpStatus.NOT_FOUND); } } else { return new
	 * ResponseEntity<RESTError>( new RESTError(HttpStatus.NOT_FOUND.value(),
	 * "Subject with id number " + subjectId +
	 * " is not assigned to teacher with id number " + teacher.getId()),
	 * HttpStatus.NOT_FOUND); } } else { return new ResponseEntity<RESTError>(new
	 * RESTError(HttpStatus.NOT_FOUND.value(), "Teacher not found"),
	 * HttpStatus.NOT_FOUND); } } else { return new ResponseEntity<RESTError>(new
	 * RESTError(HttpStatus.NOT_FOUND.value(), "Subject with id number " + subjectId
	 * + " not found"), HttpStatus.NOT_FOUND); }
	 * 
	 * 
	 * } else { return new ResponseEntity<RESTError>(new
	 * RESTError(HttpStatus.UNAUTHORIZED.value(),
	 * "You are unauthorized to view grades for subject with id number " + subjectId
	 * ), HttpStatus.UNAUTHORIZED); }
	 * 
	 * } else { return new ResponseEntity<RESTError>(new
	 * RESTError(HttpStatus.NOT_FOUND.value(), "Subject with id number " + subjectId
	 ...
	 * 
	 * } else { return new ResponseEntity<RESTError>( new
	 * RESTError(HttpStatus.NOT_FOUND.value(), "Student with id number " + studentId
	 * + " not found"), HttpStatus.NOT_FOUND); }
	 * 
	 * }
	 */
